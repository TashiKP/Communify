...
import {
  ...
  ViewStyle,
  TextStyle,
} from 'react-native'; // Removed TextInput, Switch as they are not used here
...
const ChildProfileSection: React.FC<ChildProfileSectionProps> = ({
  ...
  const profileOptions: {
    level: AsdLevel | 'noAsd';
    labelKey: string;
    icon: any;
  }[] = [
    {
      level: 'high',
      labelKey: 'parentalControls.childProfile.level3Label',
      icon: faHeart,
    },
    {
      level: 'medium',
      labelKey: 'parentalControls.childProfile.level2Label',
      icon: faChild,
    },
    {
      level: 'low',
      labelKey: 'parentalControls.childProfile.level1Label',
      icon: faUserShield,
    },
    {
      level: 'noAsd',
      labelKey: 'parentalControls.childProfile.noNeedsLabel',
      icon: faUserShield,
    }, // 'noAsd' is a string value representing null in the UI
  ];
    ...
    <View style={[styles.defaultSectionCard, sectionStyle]}>
      ...
      <View style={[styles.defaultCardHeader, headerStyle]}>
        ...
        <Text style={[styles.defaultSectionTitle, titleStyle]}>
          {t('parentalControls.childProfile.sectionTitle')}
        </Text>
      </View>
      ...
      <View style={styles.optionsList}>
        {profileOptions.map(({level, labelKey, icon}) => {
          ...
          return (
            <TouchableOpacity
              ...
              onPress={() =>
                onSettingChange('asdLevel', valueToCompareWithSettings)
              } // Pass null if 'noAsd'
              activeOpacity={0.7}
              accessibilityLabel={t(
                'parentalControls.childProfile.optionAccessibilityLabel',
                {profile: label},
              )}
        ...
        {settings.asdLevel !== null && ( // Show clear button only if a level (not 'noAsd'/null) is selected
          <TouchableOpacity
            ...
            activeOpacity={0.7}
            accessibilityLabel={t(
              'parentalControls.childProfile.clearAccessibilityLabel',
            )}
...
const createThemedStyles = (
  ...
  return StyleSheet.create({
    ...
    defaultSectionTitle: {
      // This style is passed from ParentalControls, providing a fallback

...
// --- API Service ---
import apiService, { handleApiError } from '../services/apiService'; // MODIFIED IMPORT
...
import { ParentalSettingsData, AsdLevel, DayOfWeek } from '../services/apiService'; // Assuming types are exported from apiService or a shared types file
...
const ParentalControls: React.FC<ParentalControlsProps> = ({
    ...
    useEffect(() => {
        const fetchSettings = async () => {
            ...
            try {
                ...
                if(isMountedRef.current) {
                    Alert.alert(t('common.error', 'Error'), t('parentalControls.errors.fetchFail', 'Could not load settings. Using defaults.'));
    ...
    }, [t, i18n.isInitialized, initialSettings]);

    useEffect(() => { checkPasscodeStatus(); }, [checkPasscodeStatus]);
    ...
    const togglePasscodeSetup = useCallback(() => { setShowPasscodeSetup(prev => { const nextState = !prev; if (nextState) { setCurrentPasscode(''); setNewPasscode(''); setConfirmPasscode(''); setPasscodeError(null); setPasscodeSuccess(null); setTimeout(() => { if (isMountedRef.current) { if (passcodeExists) currentPasscodeRef.current?.focus(); else newPasscodeRef.current?.focus(); }}, 150); } else { Keyboard.dismiss(); } return nextState; }); }, [passcodeExists]);
    const handleSettingChange = useCallback(<K extends keyof ParentalSettingsData>(key: K, value: ParentalSettingsData[K]) => { if (key === 'requirePasscode' && value === true && !passcodeExists) { Alert.alert(t('parentalControls.passcodeRequiredTitle'), t('parentalControls.passcodeRequiredMessageToEnable'), [{ text: t('common.ok'), onPress: togglePasscodeSetup }]); return; } setLocalSettings(prev => { if (key === 'dailyLimitHours') { const numericValue = String(value); const filteredValue = numericValue.replace(/[^0-9]/g, ''); const num = parseInt(filteredValue, 10); let finalValue: string; if (filteredValue === '') finalValue = ''; else if (!isNaN(num)) finalValue = num >= 0 && num <= 24 ? num.toString() : (num > 24 ? '24' : prev.dailyLimitHours); else finalValue = prev.dailyLimitHours; return { ...prev, dailyLimitHours: finalValue }; } return { ...prev, [key]: value }; }); }, [passcodeExists, togglePasscodeSetup, t]);
    const handleDowntimeDayToggle = useCallback((day: DayOfWeek) => { setLocalSettings(prev => ({ ...prev, downtimeDays: prev.downtimeDays.includes(day) ? prev.downtimeDays.filter(d => d !== day) : [...prev.downtimeDays, day] })); }, []);
    const handleReset = () => { if(hasUnsavedChanges) { Alert.alert(t('parentalControls.resetConfirmTitle'),t('parentalControls.resetConfirmMessage'),[{ text: t('common.cancel'), style: "cancel" },{ text: t('common.reset'), style: "destructive", onPress: () => { setLocalSettings(originalSettings); if (showPasscodeSetup) togglePasscodeSetup(); }}]); } else { setLocalSettings(originalSettings); if (showPasscodeSetup) togglePasscodeSetup(); }};
    const handleAttemptClose = useCallback(() => { if (hasUnsavedChanges) Alert.alert(t('parentalControls.unsavedChangesTitle'),t('parentalControls.unsavedChangesMessage'),[{ text: t('common.cancel'), style: "cancel" },{ text: t('common.discard'), style: "destructive", onPress: onClose }]); else onClose(); }, [hasUnsavedChanges, onClose, t]);

    const handleSaveChanges = async () => {
        if (!hasUnsavedChanges || isSaving || isLoadingPasscodeStatus || isLoadingAppearance || isLoadingApiSettings) return;
        ...
        try {
            ...
            if (isMountedRef.current) {
                ...
                Alert.alert(t('parentalControls.saveSuccessTitle', 'Success'), t('parentalControls.saveSuccessMessage', 'Settings saved successfully.'));
                onClose();
            }
        } catch (error) {
            console.error("Error saving parental controls to API:", error);
            const extractedError = handleApiError(error); // Correctly using imported function
            if (isMountedRef.current) {
                Alert.alert(
    ...
    const handleSetOrUpdatePasscode = useCallback(async () => { Keyboard.dismiss(); setPasscodeError(null); setPasscodeSuccess(null); if (passcodeExists && !currentPasscode) { setPasscodeError(t('parentalControls.passcode.errorEnterCurrent')); return; } if (!newPasscode || newPasscode.length < 4) { setPasscodeError(t('parentalControls.passcode.errorNewMinLength')); return; } if (newPasscode !== confirmPasscode) { setPasscodeError(t('parentalControls.passcode.errorMismatch')); return; } setIsSettingPasscode(true); try { if (passcodeExists) { const verified = await KeychainService.verifyPasscode(currentPasscode); if (!verified) { setPasscodeError(t('parentalControls.passcode.errorIncorrectCurrent')); setIsSettingPasscode(false); return; }} const success = await KeychainService.setPasscode(newPasscode); if (success) { const wasFirstPasscode = !passcodeExists; setPasscodeSuccess(t('parentalControls.passcode.successSetUpdate')); setPasscodeExists(true); handleSettingChange('requirePasscode', true); setTimeout(() => { if(isMountedRef.current) { togglePasscodeSetup(); if (wasFirstPasscode) { Alert.alert(t('parentalControls.passcode.successTitle'), t('parentalControls.passcode.successRequireEnabledMessage')); }}}, 1500); } else { setPasscodeError(t('parentalControls.passcode.errorSaveFailed')); }} catch (error) { console.error("Error setting/updating passcode:", error); setPasscodeError(t('parentalControls.passcode.errorUnexpected')); } finally { if(isMountedRef.current) setIsSettingPasscode(false); }}, [passcodeExists, currentPasscode, newPasscode, confirmPasscode, togglePasscodeSetup, t, handleSettingChange]);
    const handleRemovePasscodeClick = useCallback(async () => { Keyboard.dismiss(); setPasscodeError(null); setPasscodeSuccess(null); if (!currentPasscode) { setPasscodeError(t('parentalControls.passcode.errorEnterCurrentToRemove')); return; } Alert.alert( t('parentalControls.passcode.removeConfirmTitle'), t('parentalControls.passcode.removeConfirmMessage'), [{ text: t('common.cancel'), style: "cancel" },{ text: t('common.remove'), style: "destructive", onPress: async () => { setIsSettingPasscode(true); try { const verified = await KeychainService.verifyPasscode(currentPasscode); if (!verified) { setPasscodeError(t('parentalControls.passcode.errorIncorrectCurrent')); setIsSettingPasscode(false); return; } const success = await KeychainService.resetPasscode(); if(success) { setPasscodeSuccess(t('parentalControls.passcode.successRemoved')); setPasscodeExists(false); handleSettingChange('requirePasscode', false); setTimeout(() => { if(isMountedRef.current){ togglePasscodeSetup(); Alert.alert(t('parentalControls.passcode.successRemovedTitle'), t('parentalControls.passcode.successRequireDisabledMessage')); }}, 1500); } else { setPasscodeError(t('parentalControls.passcode.errorRemoveFailed')); }} catch (error) { console.error("Error removing passcode:", error); setPasscodeError(t('parentalControls.passcode.errorUnexpected')); } finally { if(isMountedRef.current) setIsSettingPasscode(false); }}}]); }, [currentPasscode, togglePasscodeSetup, t, handleSettingChange]);
    ...
    const handleAddNotifyEmail = useCallback(() => { const trimmedEmail = newNotifyEmail.trim(); if (!trimmedEmail) return; if (!emailRegex.test(trimmedEmail)) { Alert.alert(t('parentalControls.errors.invalidEmail')); return; } const lowerCaseEmail = trimmedEmail.toLowerCase(); if (localSettings.notifyEmails.some(email => email.toLowerCase() === lowerCaseEmail)) { Alert.alert(t('parentalControls.errors.duplicateEmail')); return; } handleSettingChange('notifyEmails', [...localSettings.notifyEmails, trimmedEmail]); setNewNotifyEmail(''); setShowAddEmailInput(false); Keyboard.dismiss(); }, [newNotifyEmail, localSettings.notifyEmails, handleSettingChange, t]);
    ...
    const isSaveDisabled = isSaving || !hasUnsavedChanges || isLoadingAnyInitialData || isSettingPasscode || (localSettings.requirePasscode && !passcodeExists);
    const isResetDisabled = isSaving || !hasUnsavedChanges || isLoadingAnyInitialData || isSettingPasscode;

    if (!i18n.isInitialized || typeof t !== 'function') { return (<SafeAreaView style={styles.screenContainer}><View style={[styles.contentArea, {justifyContent: 'center', alignItems: 'center'}]}><ActivityIndicator size="large" color={theme.primary || '#0077b6'} /><Text style={styles.loadingText}>Loading Interface...</Text></View></SafeAreaView>); }
    if (isLoadingAnyInitialData && !showPasscodeSetup) {
        return (
            <SafeAreaView style={styles.screenContainer}>
                 <View style={styles.header}><View style={styles.headerButton} /><View style={styles.titleContainer}><Text style={styles.title}>{t('parentalControls.title')}</Text></View><View style={styles.headerButton} /></View>
    ...
    return (
        <SafeAreaView style={styles.screenContainer}>
            <View style={styles.header}>
                 ...
                 <TouchableOpacity style={[styles.headerButton, isSaveDisabled && styles.buttonDisabled]} onPress={handleSaveChanges} disabled={isSaveDisabled} hitSlop={hitSlop} accessibilityLabel={t('common.saveSettings')} accessibilityState={{ disabled: isSaveDisabled }} >
                    {isSaving ? <ActivityIndicator size="small" color={theme.white} /> : <FontAwesomeIcon icon={faSave} size={(fonts.h2 || 20) * 0.9} color={!isSaveDisabled ? theme.white : (theme.disabled || '#ccc')} /> }
                 </TouchableOpacity>
            </View>

            <ScrollView style={styles.contentArea} contentContainerStyle={styles.scrollContentContainer} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
                ...
                <SecuritySection settings={localSettings} passcodeExists={passcodeExists} onSettingChange={handleSettingChange} onTogglePasscodeSetup={togglePasscodeSetup} isLoadingPasscodeStatus={isLoadingPasscodeStatus} t={t} sectionStyle={styles.sectionCard} headerStyle={styles.sectionHeader} titleStyle={styles.sectionTitle} iconStyle={styles.sectionIcon} />
                {showPasscodeSetup && (<View style={styles.passcodeSetupCard}><Text style={styles.passcodeSetupTitle}>{passcodeExists ? t('parentalControls.passcode.changeTitle') : t('parentalControls.passcode.setTitle')}</Text>{passcodeExists && (<View style={styles.inputGroup}><Text style={styles.inputLabel}>{t('parentalControls.passcode.currentLabel')}</Text><TextInput ref={currentPasscodeRef} style={styles.textInput} value={currentPasscode} onChangeText={setCurrentPasscode} keyboardType="number-pad" secureTextEntry maxLength={10} returnKeyType="next" onSubmitEditing={() => newPasscodeRef.current?.focus()} blurOnSubmit={false} autoFocus={true} placeholderTextColor={theme.disabled} selectionColor={theme.primary} /></View>)}<View style={styles.inputGroup}><Text style={styles.inputLabel}>{t('parentalControls.passcode.newLabel')}</Text><TextInput ref={newPasscodeRef} style={styles.textInput} value={newPasscode} onChangeText={setNewPasscode} keyboardType="number-pad" secureTextEntry maxLength={10} returnKeyType="next" onSubmitEditing={() => confirmPasscodeRef.current?.focus()} blurOnSubmit={false} autoFocus={!passcodeExists} placeholderTextColor={theme.disabled} selectionColor={theme.primary} /></View><View style={styles.inputGroup}><Text style={styles.inputLabel}>{t('parentalControls.passcode.confirmLabel')}</Text><TextInput ref={confirmPasscodeRef} style={styles.textInput} value={confirmPasscode} onChangeText={setConfirmPasscode} keyboardType="number-pad" secureTextEntry maxLength={10} returnKeyType="done" onSubmitEditing={handleSetOrUpdatePasscode} placeholderTextColor={theme.disabled} selectionColor={theme.primary}/></View>{passcodeError && <Text style={styles.passcodeFeedbackError}>{passcodeError}</Text>}{passcodeSuccess && <Text style={styles.passcodeFeedbackSuccess}>{passcodeSuccess}</Text>}<View style={styles.passcodeActionsRow}>{passcodeExists && ( <TouchableOpacity style={[styles.passcodeActionButton, styles.removeButton]} onPress={handleRemovePasscodeClick} disabled={isSettingPasscode} accessibilityLabel={t('common.remove')}> {isSettingPasscode ? <ActivityIndicator size="small" color={ERROR_COLOR_HEX}/> : <Text style={styles.removeButtonText}>{t('common.remove')}</Text>} </TouchableOpacity> )}                            <TouchableOpacity style={[styles.passcodeActionButton, styles.cancelButton]} onPress={togglePasscodeSetup} disabled={isSettingPasscode} accessibilityLabel={t('common.cancel')}><Text style={styles.cancelButtonText}>{t('common.cancel')}</Text></TouchableOpacity><TouchableOpacity style={[styles.passcodeActionButton, styles.saveButton,(isSettingPasscode || !newPasscode || newPasscode.length < 4 || newPasscode !== confirmPasscode || (passcodeExists && !currentPasscode)) && styles.buttonDisabled]} onPress={handleSetOrUpdatePasscode} disabled={isSettingPasscode || !newPasscode || newPasscode.length < 4 || newPasscode !== confirmPasscode || (passcodeExists && !currentPasscode) } accessibilityLabel={passcodeExists ? t('common.update') : t('common.set')} >{isSettingPasscode ? <ActivityIndicator size="small" color={theme.white}/> : <Text style={styles.saveButtonText}>{passcodeExists ? t('common.update') : t('common.set')}</Text>}</TouchableOpacity></View></View>)}
                <View style={styles.actionsContainer}><TouchableOpacity style={[styles.resetButton, isResetDisabled && styles.buttonDisabled]} onPress={handleReset} disabled={isResetDisabled} accessibilityLabel={t('common.resetChanges')}><FontAwesomeIcon icon={faUndo} size={(fonts.label || 16) * 0.9} color={!isResetDisabled ? theme.textSecondary : (theme.disabled || '#ccc')} style={styles.buttonIcon}/><Text style={[styles.resetButtonText, isResetDisabled && styles.textDisabled]}>{t('common.resetChanges')}</Text></TouchableOpacity></View>
            </ScrollView>
...
const createThemedStyles = ( theme: ThemeColors, fonts: FontSizes, currentLanguage: string ) => {
    ...
    return StyleSheet.create({
        ...
        passcodeSetupCard: { backgroundColor: theme.card, borderRadius: 12, padding: 20, marginBottom: 20, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: theme.isDark ? 0.35 : 0.15, shadowRadius: 4, elevation: 3, borderWidth: theme.isDark ? StyleSheet.hairlineWidth : 0, borderColor: theme.border },
        passcodeSetupTitle: { ...labelStyles, fontSize:labelFontSize, fontWeight: '600', color: theme.textSecondary, marginBottom: 25, textAlign: 'center' },
        inputGroup: { marginBottom: 18 },
        inputLabel: { ...captionStyles, fontWeight: '500', color: theme.textSecondary, marginBottom: 6 },
        textInput: { ...bodyStyles, backgroundColor: theme.background, height: 48, borderColor: theme.border, borderWidth: 1, borderRadius: 8, paddingHorizontal: 12, color: theme.text },
        passcodeFeedbackError: { ...captionStyles, color: ERROR_COLOR_HEX, textAlign: 'center', marginTop: 8, marginBottom: 12, fontWeight: '500' },
        passcodeFeedbackSuccess: { ...captionStyles, color: SUCCESS_COLOR_HEX, textAlign: 'center', marginTop: 8, marginBottom: 12, fontWeight: '500' },
        passcodeActionsRow: { flexDirection: 'row', justifyContent: 'flex-end', marginTop: 20, gap: 10 },
        passcodeActionButton: { paddingVertical: 10, paddingHorizontal: 15, borderRadius: 8, alignItems: 'center', justifyContent: 'center', minHeight: 44, flexShrink: 1 },
        ...
        resetButtonText: { ...labelStyles, fontSize: labelFontSize, color: theme.textSecondary, fontWeight: '500', textDecorationLine: 'underline' },

...
import apiService, { handleApiError, ParentalSettingsData, DayOfWeek } from '../services/apiService';
import { useAppearance, ThemeColors, FontSizes } from '../context/AppearanceContext'; // Ensure currentLanguage is not expected here
import { getLanguageSpecificTextStyle } from '../styles/typography';

// Child Section Components
import ContentFilteringSection from './ParentalControls/ContentFilteringSection';
import ScreenTimeSection, { ReportEventType } from './ParentalControls/ScreenTimeSection';
...
const ParentalControls: React.FC<ParentalControlsProps> = ({
  ...
  const stableOnSaveSuccess = useCallback((settings: ParentalSettingsData) => propOnSaveSuccess?.(settings), [propOnSaveSuccess]);
  const settingsHookRef = useRef<ReturnType<typeof useParentalSettings> | null>(null);
  ...
  const timePickerHook = useTimePicker({
    ...
    onTimeSelected: (target, time) => settingsHook.handleSettingChange(target as keyof ParentalSettingsData, time),
  });

  const notifyEmailFormHook = useNotifyEmailForm({
    getNotifyEmails: () => settingsHook.localSettings.notifyEmails,
    onEmailsChange: (emails) => settingsHook.handleSettingChange('notifyEmails', emails),
    t,
  });
  ...
  useEffect(() => {
    if (!passcodeHook.isLoadingPasscodeStatus && !settingsHook.isLoadingApiSettings && i18n.isInitialized && typeof t === 'function') {
      if (settingsHook.localSettings.requirePasscode && !passcodeHook.passcodeExists && !passcodeHook.showPasscodeSetup) {
        Alert.alert(
    ...
    passcodeHook.isLoadingPasscodeStatus, settingsHook.isLoadingApiSettings,
    settingsHook.localSettings.requirePasscode, passcodeHook.passcodeExists,
    passcodeHook.showPasscodeSetup, t, i18n.isInitialized // Added i18n.isInitialized
  ]);
    ...
    if (settingsHook.hasUnsavedChanges) {
      Alert.alert(
        ...
        [
          ...
          { text: t('common.discard'), style: 'destructive', onPress: stableOnClose }, // Use stable callback
        ]
      );
    } else {
  ...
  }, [settingsHook.hasUnsavedChanges, settingsHook.resetSettings, passcodeHook.showPasscodeSetup, passcodeHook.togglePasscodeSetup, t]);

  const handleSendEmailReport = useCallback(async (
    subject: string,
    body: string,
    recipients: string[],
    ...
    if (!recipients || recipients.length === 0) {
      console.log(`Email report for ${eventType} not sent: No recipients configured.`);
      return;
    }
    try {
      const response = await apiService.sendParentalControlReportEmail(subject, body, recipients, eventType);
      if (response.success) console.log(`Email report for ${eventType} sent successfully.`);
      else console.error(`Failed to send email report for ${eventType}: ${response.message}`);
    } catch (error) {
      console.error(`Error in handleSendEmailReport for ${eventType}:`, handleApiError(error).message);
    }
  ...
  const isLoadingAnyInitialData = isLoadingAppearance || settingsHook.isLoadingApiSettings || passcodeHook.isLoadingPasscodeStatus;
  const isSaveDisabled =
    ...
    (settingsHook.localSettings.requirePasscode && !passcodeHook.passcodeExists);
  const isResetDisabled = settingsHook.isSaving || !settingsHook.hasUnsavedChanges || isLoadingAnyInitialData || passcodeHook.isSettingPasscode;
  ...
  if ((settingsHook.isLoadingApiSettings || passcodeHook.isLoadingPasscodeStatus) && !passcodeHook.showPasscodeSetup) {
    return (
      <SafeAreaView style={styles.screenContainer}>
        <ParentalControlsHeader
            title={t('parentalControls.title')}
            onClose={handleAttemptClose}
            onSave={() => { /* No save action possible while loading */}}
            isSaveDisabled={true}
            isSaving={false}
            t={t}
        />
        ...
        <View style={styles.actionsContainer}>
          ...
          >
            <FontAwesomeIcon
              ...
              color={!isResetDisabled ? (theme.textSecondary || '#555') : (theme.disabled || '#ccc')}
              style={styles.buttonIcon}
            />
            <Text style={[styles.resetButtonText, {color: !isResetDisabled ? (theme.textSecondary || '#555') : (theme.disabled || '#ccc')}, isResetDisabled && styles.textDisabled]}>
...
const createThemedStyles = (theme: ThemeColors, fonts: FontSizes, currentLanguage: string) => {
  ...
  return StyleSheet.create({
    ...
    centeredContent: { // Added for loading states
        justifyContent: 'center',
        alignItems: 'center',
    },
    scrollContentContainer: { flexGrow: 1, padding: 15, paddingBottom: 40 },
    loadingText: { ...bodyStyles, color: theme.textSecondary, marginTop: 15, textAlign: 'center' },
    sectionCard: {

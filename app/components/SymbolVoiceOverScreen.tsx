...
import {
  SymbolVoiceOverScreenProps,
  TtsVoice,
  VoiceSettingData,
} from './SymbolVoiceOver/types';
...
const SymbolVoiceOverScreen: React.FC<SymbolVoiceOverScreenProps> = ({
  ...
  useEffect(() => {
    ...
    let startListener: any = null,
      finishListener: any = null,
      cancelListener: any = null;
    const initTts = async () => {
      ...
      try {
        ...
        if (isMountedRef.current) {
          ...
          if (
            currentVoiceId &&
            usableVoices.some(v => v.id === currentVoiceId)
          ) {
            voiceToSet = currentVoiceId;
          } else if (usableVoices.length > 0) {
            const defaultUsEnglish = usableVoices.find(v =>
              v.language.startsWith('en-US'),
            );
            voiceToSet = defaultUsEnglish
              ? defaultUsEnglish.id
              : usableVoices[0].id;
          }
        ...
        if (
          error.message?.indexOf('TTS engine is not ready') === -1 &&
          isMountedRef.current
        ) {
          Alert.alert(
            t('voiceSettings.errors.ttsInitTitle'),
            t('voiceSettings.errors.ttsInitMessage', {message: error.message}),
          );
        }
      } finally {
  ...
  const handleSettingChange = useCallback(
    <K extends keyof VoiceSettingData>(key: K, value: VoiceSettingData[K]) => {
  ...
  const handlePreview = async () => {
    ...
    if (isLoadingVoices || availableVoices.length === 0) {
      Alert.alert(
        t('voiceSettings.errors.previewTitle'),
        isLoadingVoices
          ? t('voiceSettings.errors.voicesLoading')
          : t('voiceSettings.errors.noVoices'),
      );
      return;
    }
    ...
    if (!voiceIdToUse && availableVoices.length > 0) {
      ...
      Alert.alert(
        t('voiceSettings.voiceSelectedTitle'),
        t('voiceSettings.voiceSelectedMessage', {name: selectedVoiceName}),
        [{text: t('common.ok')}],
      );
    } else if (!voiceIdToUse) {
      Alert.alert(
        t('voiceSettings.errors.previewTitle'),
        t('voiceSettings.errors.noVoiceSelected'),
      );
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        t('voiceSettings.errors.previewTitle'),
        t('voiceSettings.errors.previewFail'),
      );
      if (isMountedRef.current) setIsSpeaking(false);
    }
  };
    ...
    Alert.alert(
      ...
      [
        {text: t('common.cancel'), style: 'cancel'},
        {
          text: t('common.reset'),
          style: 'destructive',
          onPress: () => setLocalSettings(originalSettings),
        },
      ],
    );
  };
    ...
    <SafeAreaView style={styles.screenContainer}>
      ...
      <ScrollView
        ...
        <VoiceSelection
          ...
          onVoiceChange={(voiceId: string | null) =>
            handleSettingChange('selectedVoiceId', voiceId)
          }
          styles={styles}
          theme={theme}
          t={t}
        ...
        <Preview
          ...
          isPreviewDisabled={
            isSaving || isLoading || !localSettings.selectedVoiceId
          }

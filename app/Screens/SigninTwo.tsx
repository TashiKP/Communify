...
const SigninTwo: React.FC = () => {
        ...
        try {
            ...
            if (setUser && authToken && registeredUser && registeredUser.id) {
                ...
                const basicUserDataForStorage: Omit<AuthUser, 'localAvatarPath'> = {
                    id: userForAuthContext.id, email: userForAuthContext.email, name: userForAuthContext.name,
                    age: userForAuthContext.age, gender: userForAuthContext.gender,
                    userType: userForAuthContext.userType, isActive: userForAuthContext.isActive,
                };
            ...
            if (errorInfo.details) console.warn('[SigninTwo] Validation Details from API:', errorInfo.details);
            
            if (registeredUser && !authToken) {
                Alert.alert("Registration Succeeded, Login Failed", "Your account was created, but auto-login failed. Please log in manually. Initial preferences might not have saved.", [{text: 'Login', onPress: () => navigation.replace('Login')}]);
    ...
    return (
        <SafeAreaView style={styles.safeArea}>
            ...
            >
                <AuthScreenHeader title="Personalize Experience" onBackPress={handleGoBack} />

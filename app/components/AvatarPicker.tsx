...
const AvatarPicker: React.FC<AvatarPickerProps> = ({
  ...
  const [currentAvatarUri, setCurrentAvatarUri] = useState<string | undefined>(
  ...
  const deletePreviousAvatar = async (filePath: string | undefined) => {
    ...
    if (
      ...
      try {
        ...
        console.error(
          '[AvatarPicker] Error deleting previous avatar:',
          unlinkError,
        );
  ...
  const pickAndSaveImage = useCallback(async () => {
    ...
    launchImageLibrary(options, async (response: ImagePickerResponse) => {
      ...
      if (response.errorCode) {
        console.error(
          '[AvatarPicker] ImagePicker Error:',
      ...
      if (response.assets && response.assets[0]?.uri) {
        ...
        try {
          ...
          console.log(
            `[AvatarPicker] Resizing to max ${maxWidth}x${maxHeight}, Q${compressQuality}, Fmt:${outputFormat}`,
          );
          ...
          console.log(
            `[AvatarPicker] Resized image size: ${(
              resizedImage.size / 1024
            ).toFixed(2)} KB`,
          );
          ...
          const rnfsSourcePath = sourceUriForCopy.startsWith('file://')
            ? sourceUriForCopy.substring(7)
            : sourceUriForCopy;

          console.log(
            `[AvatarPicker] Copying RESIZED from ${rnfsSourcePath} to ${permanentPath}`,
          );
          await RNFS.copyFile(rnfsSourcePath, permanentPath);
          console.log(
            '[AvatarPicker] RESIZED Image copied successfully to:',
            permanentPath,
          );
          ...
          console.error(
            '[AvatarPicker] Error processing/resizing/copying file:',
            processingError,
          );
          ...
          if (
            resizedImage?.uri &&
            resizedImage.uri.includes(RNFS.CachesDirectoryPath)
          ) {
            ...
            RNFS.exists(resizeCachePath).then(exists => {
              if (exists) {
                RNFS.unlink(resizeCachePath)
                  .then(() =>
                    console.log(
                      '[AvatarPicker] Deleted temp resized file:',
                      resizeCachePath,
                    ),
                  )
                  .catch(err =>
                    console.error(
                      '[AvatarPicker] Failed to delete temp resized file:',
                      err,
                    ),
                  );
              }
        ...
        console.log(
          '[AvatarPicker] Image picker response missing assets or URI.',
        );
  ...
  const dynamicStyles = React.useMemo(
    ...
    [size, placeholderColor, cameraIconBackgroundColor, cameraIconColor],
  ); // Dependencies for dynamic styles

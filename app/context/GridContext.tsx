...
const GridContext = createContext<GridContextProps>({
  ...
  setGridLayout: async () => {
    console.warn('setGridLayout called before Provider');
  },
...
export const GridProvider: React.FC<GridProviderProps> = ({children}) => {
  ...
  useEffect(() => {
    ...
    const loadLayout = async () => {
      ...
      try {
        const storedLayout = await AsyncStorage.getItem(
        ...
        if (isMounted) {
          if (
            ...
            console.log(
              'GridContext: Loaded layout from storage:',
              storedLayout,
            );
          } else {
            console.log(
              'GridContext: No valid layout in storage, using default.',
            );

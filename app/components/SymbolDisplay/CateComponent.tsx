...
const CateComponent: React.FC<CateComponentProps> = React.memo(
  ({
    ...
    useEffect(() => {
      ...
      console.log(
        `CateComponent: Mounted/Updated for keyword "${keyword}", iconKeyword: "${iconKeywordForArasaac}"`,
      );
    ...
    useEffect(() => {
        ...
        try {
          ...
          if (isMountedRef.current) {
            if (response.data?.[0]?._id) {
              setPictogramUrl(
                `https://static.arasaac.org/pictograms/${response.data[0]._id}/${response.data[0]._id}_300.png`,
              );
            } else {
              setPictogramUrl(null);
          ...
          if (axios.isCancel(err)) {
            console.log(
              `CateComponent: Arasaac fetch cancelled for '${iconKeywordForArasaac}'.`,
            );
            return;
          }
          if (isMountedRef.current) {
            console.error(
              `CateComponent: Arasaac fetch error for '${iconKeywordForArasaac}':`,
      ...
      return () => {
        console.log(
          `CateComponent: Cleanup for fetch effect (keyword: ${iconKeywordForArasaac}). Cancelling request.`,
        );
        currentToken.cancel(
          'Component unmounted or dependencies changed, cancelling pictogram fetch.',
        );
      };

...
const Menu: React.FC<MenuProps> = ({
  ...
  const [parentalSettings, setParentalSettings] =
    useState<ParentalSettingsData>(defaultParentalSettings);
  const [isLoadingParentalSettings, setIsLoadingParentalSettings] =
    useState(true);
  const [isCheckingPasscodeForMenu, setIsCheckingPasscodeForMenu] =
    useState(false);
  ...
  const [localKeychainPasscodeExists, setLocalKeychainPasscodeExists] =
    useState(false);
  ...
  const isLoadingInitialSettings =
    isLoadingAppearance || isLoadingParentalSettings;
  ...
  const styles = useMemo(
    () => createThemedMenuStyles(theme, fonts, currentLanguage),
  ...
  useEffect(() => {
    const loadAllMenuData = async () => {
      ...
      try {
        const [fetchedParentalSettings, keychainHasPasscode] =
          await Promise.all([
        ...
        setParentalSettings({
          ...
          ...fetchedParentalSettings,
        });
        AsyncStorage.setItem(
          PARENTAL_SETTINGS_STORAGE_KEY,
          JSON.stringify(fetchedParentalSettings),
        ).catch(cacheError => {
  ...
  const handleMenuPress = useCallback(
    async (itemId: string) => {
      ...
      if (isLoadingInitialSettings) {
        Alert.alert(
          t('menu.loadingTitle', 'Loading...'),
          t('menu.loadingMessage', 'Settings are loading, please wait.'),
        );
        return;
      }
      ...
      if (protectedItems.includes(itemId)) {
        if (parentalSettings.requirePasscode) {
          ...
          try {
            const backendPasscodeStatus =
              await apiService.checkBackendHasParentalPasscode();
            if (!isMountedRef.current) return;
            ...
            if (backendPasscodeStatus.exists) {
              ...
              Alert.alert(
                ...
                t(
                  'menu.errors.passcodeRequiredButNotConfiguredMenu',
                  'A passcode is required by your settings, but it appears none is configured on the server. Please visit Parental Controls to set up a passcode.',
                ),
                [{text: t('common.ok')}],
              );
            ...
            if (!isMountedRef.current) return;
            console.error(
              'Menu: Error checking backend passcode status for menu access:',
              error,
            );
            Alert.alert(
              t('common.error'),
              t(
                'menu.errors.passcodeStatusCheckFailedMenu',
                'Could not verify passcode status. Please try again.',
              ),
            );
            setTargetModalId(null);
          } finally {
  ...
  const handleParentalSettingsUpdate = useCallback(
    async (updatedSettingsFromApi: ParentalSettingsData) => {
  ...
  const memoizedParentalSettings = useMemo(
    () => parentalSettings,
    [parentalSettings],
  );
  ...
  const renderMenuContent = () => (
    <ScrollView
      ...
      {menuItems.map(item => {
        ...
        const isDisabledByLoading =
          isLoadingInitialSettings ||
          (isCheckingPasscodeForMenu && targetModalId === item.id);
        ...
        return (
          <TouchableOpacity
            ...
            activeOpacity={0.6}
            disabled={
              isDisabledByLoading && isProtectedItem && item.id !== 'about'
            }
            ...
            <View
              style={[
                ...
                isDisabledByLoading &&
                  isProtectedItem &&
                  item.id !== 'about' &&
                  styles.disabledIconWrapper,
              ]}>
              {isCheckingPasscodeForMenu &&
              targetModalId === item.id &&
              isProtectedItem ? (
    ...
    <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
      <Animated.View style={[styles.overlay, {opacity: overlayAnim}]}>
        <TouchableOpacity
          ...
          activeOpacity={1}
          onPress={closeMenu}
          accessibilityLabel={t('menu.closeMenuAccessibilityLabel')}
        />
      </Animated.View>
      <Animated.View
      ...
      <Modal
        visible={activeModal === 'childInfo'}
        animationType="slide"
        transparent={false}
        onRequestClose={handleCloseSubModal}>
        {activeModal === 'childInfo' && (
      ...
      <Modal
        ...
        {activeModal === 'voiceover' && (
          <SymbolVoiceOverScreen
            onClose={handleCloseSubModal}
            initialSettings={currentTtsSettings}
            onSave={onTtsSettingsSave}
          />
        )}
      </Modal>
      <Modal
        ...
        {activeModal === 'parental' && (
          <ParentalControls
            ...
            onSaveSuccess={handleParentalSettingsUpdate}
          />
        )}
      </Modal>
      <Modal
      ...
      <PasscodePromptModal
        visible={isPasscodePromptVisible}

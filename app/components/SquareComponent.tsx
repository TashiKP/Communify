...
const SquareComponent: React.FC<SquareComponentProps> = React.memo(
  ({ keyword, displayText, language, imageUri, onPress, size }) => {
    // --- Hooks ---
    const { theme, fonts } = useAppearance();
    const { t, i18n } = useTranslation(); // <-- Get t and i18n for language

    // --- Dynamic Styles ---
    const styles = useMemo(
      () => createThemedStyles(theme, fonts, size, i18n.language),
      [theme, fonts, size, i18n.language] // Added i18n.language to dependencies
    );
          ...
          <View style={styles.contentArea}>
            ...
            {!loading && !imageUri && pictogramUrl && !error && (
              <Image
                ...
                accessibilityLabel="" // Decorative if text label present
                onError={() => {
                  if (isMountedRef.current) setError(t('squareComponent.arasaacImageError')); // Use t()
                }}
              />
            )}
            {/* Fallback Text/Icon */}
...
const createThemedStyles = (
  ...
  // Get language-specific text styles for 'caption' type
  ...
  return StyleSheet.create({
    ...
    fallbackText: {
      ...textStyle, // Apply fontFamily, fontSize, lineHeight from typography
      fontWeight: '500',

...
export const LanguageProvider: React.FC<LanguageProviderProps> = ({
  ...
  const [isTranslatingSymbols, setIsTranslatingSymbols] =
  ...
  useEffect(() => {
    const handleLanguageChanged = (lng: string) => {
      console.log(
        `LanguageContext: Listener notified of language change to ${lng}`,
      );
  ...
  const changeLanguage = useCallback(
    (newLanguage: LanguageCode) => {
      console.log(
        `LanguageContext: User requested change to ${newLanguage} via i18next`,
      );
      ...
      if (newLanguage !== currentLanguage) {
        ...
        console.log(
          `LanguageContext: Language already set to ${newLanguage}, no change needed.`,
        );
      }
    },
    [currentLanguage],
  ); // Depend on currentLanguage to avoid unnecessary calls

  // Function to translate dynamic symbols via API
  const translateSymbolBatch = useCallback(
    async (texts: string[], targetLang?: LanguageCode): Promise<string[]> => {
      ...
      try {
        ...
        console.warn(
          'LanguageContext: translateSymbolBatch returning fallback due to result mismatch or error.',
        );
        return texts; // Fallback
      } catch (error) {
        console.error(
          'LanguageContext: Error translating symbol batch:',
          error,
        );
        return texts; // Fallback
      } finally {
    ...
    [currentLanguage],
  ); // Depend only on currentLanguage

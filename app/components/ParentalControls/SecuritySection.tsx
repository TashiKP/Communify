...
import {
  ...
  faChevronRight,
  faKey,
} from '@fortawesome/free-solid-svg-icons'; // faKey could be an alternative
...
const SecuritySection: React.FC<SecuritySectionProps> = ({
  ...
  const switchStyles = useMemo(
    () => ({
      ...
      thumbColor:
        Platform.OS === 'android' ? theme.primary || '#007aff' : undefined, // Thumb color for Android
  ...
  const handleRequirePasscodeToggle = (value: boolean) => {
    if (value && !canEnablePasscodeRequirement) {
      Alert.alert(
        t(
          'parentalControls.security.setPasscodeFirstTitle',
          'Set Passcode First',
        ),
        t(
          'parentalControls.security.setPasscodeFirstMessage',
          'Please set up a passcode before requiring it for settings changes.',
        ),
      );
      return;
    }
  ...
  const switchRowTextColor = isSwitchDisabled
    ? theme.disabled || '#aaa'
    : theme.text || '#000';
  const switchRowIconColor = isSwitchDisabled
    ? theme.disabled || '#aaa'
    : theme.textSecondary || '#555';
  const configureButtonTextColor = isConfigureButtonDisabled
    ? theme.disabled || '#aaa'
    : theme.text || '#000'; // Use theme.text for enabled
  const configureButtonIconColor = isConfigureButtonDisabled
    ? theme.disabled || '#aaa'
    : theme.textSecondary || '#555';
  const chevronColor = isConfigureButtonDisabled
    ? theme.disabled || '#aaa'
    : theme.textSecondary || '#555';
    ...
    <View style={[styles.defaultSectionCard, sectionStyle]}>
      <View style={[styles.defaultCardHeader, headerStyle]}>
        ...
        <Text style={[styles.defaultSectionTitle, titleStyle]}>
          {t('parentalControls.security.sectionTitle', 'Security & Passcode')}
        </Text>
        {isLoadingPasscodeStatus && (
          <ActivityIndicator
            size="small"
      ...
      <View
        style={[styles.settingRow, isSwitchDisabled && styles.disabledVisual]}>
        <FontAwesomeIcon
          icon={faUserShield}
          size={(fonts.body || 16) * 1.1}
          color={switchRowIconColor}
          style={styles.settingIcon}
        />
        <Text style={[styles.settingLabel, {color: switchRowTextColor}]}>
          {t(
            'parentalControls.security.requirePasscodeLabel',
            'Require Passcode for Settings',
          )}
        </Text>
        <Switch
          ...
          accessibilityLabel={t(
            'parentalControls.security.requirePasscodeAccessibilityLabel',
            'Toggle passcode requirement for settings',
          )}
          accessibilityState={{
            disabled: isSwitchDisabled,
      ...
      <View
        style={[
          styles.cardFooter,
          isConfigureButtonDisabled && styles.disabledVisual,
        ]}>
        <TouchableOpacity
          ...
          />
          <Text
            style={[styles.featureLabel, {color: configureButtonTextColor}]}>
...
const createThemedStyles = (
  ...
  return StyleSheet.create({
    ...
    cardFooter: {
      // This View wraps the TouchableOpacity
      borderTopWidth: StyleSheet.hairlineWidth,
      borderTopColor: theme.border || '#ddd',
    },
    featureRow: {
      // This is the TouchableOpacity itself
      flexDirection: 'row',
      alignItems: 'center',
    ...
    disabledVisual: {
      // Renamed from disabledOverlay for clarity
      opacity: 0.6,
    },
  });
};

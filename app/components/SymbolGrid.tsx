...
import { CategoryInfo, CustomSymbolItem, DisplayedSymbolData, SymbolGridProps, SymbolGridRef } from './SymbolDisplay/types';
import { useGrid } from '../context/GridContext';
import { useLanguage } from '../context/LanguageContext';
import { calculateItemWidth, getNumColumns } from './SymbolDisplay/gridHelpers';
import apiService, { handleApiError, StandardCategorySymbolMap, TimeContextSymbolsResponse } from '../services/apiService';
import { APP_CATEGORIES_BASE, CUSTOM_SYMBOLS_STORAGE_KEY } from './SymbolDisplay/constants';
...
const SymbolGrid = forwardRef<SymbolGridRef, SymbolGridProps>(
  ({ onSymbolPress, onCategoryNameChange }, ref) => {
    ...
    const itemWidth = useMemo(() => calculateItemWidth(gridLayout, numGridColumns, ITEM_MARGIN), [gridLayout, numGridColumns]);
    ...
    useEffect(() => {
      ...
      const baseNonStandardCategories = APP_CATEGORIES_BASE.filter(c => !c.isStandard);
      ...
      const apiGeneratedStandardCategories = apiCategoryKeys.map(key => {
        ...
        const predefinedStandardCat = APP_CATEGORIES_BASE.find(
          (bc) => bc.isStandard === true && bc.name.toLowerCase() === key.toLowerCase()
        );
        return {
          id: `cat_${key.toLowerCase()}`,
          // Ensure name is a string before calling charAt/slice
          name: predefinedStandardCat && predefinedStandardCat.name
      ...
      apiGeneratedStandardCategories.forEach(apiCat => {
        if (!combined.some(existingCat => existingCat.name.toLowerCase() === apiCat.name.toLowerCase())) {
          combined.push(apiCat);
        }
      });
      
      if (isMountedRef.current) {
        setAllDisplayCategories(combined.sort((a, b) => {
    ...
    const loadAndTranslateSymbols = useCallback(
      async (categoryKeyToLoad: string | null = null) => {
        ...
        try {
          ...
          const categoryInfo = allDisplayCategories.find(c => c.name.toLowerCase() === effectiveCategoryKey.toLowerCase());


          if (effectiveCategoryKey === 'custom') {
            symbolIdsPrefix = 'custom_sym';
            isCustomCategory = true;
            originalKeywords = localCustomSymbols.map(cs => { imageUrisMap.set(cs.name, cs.imageUri); return cs.name; });
            ...
            currentCategoryDisplayName = t(`categories.${safeCategoryName.toLowerCase()}`, {
              defaultValue: safeCategoryName.charAt(0).toUpperCase() + safeCategoryName.slice(1).toLowerCase(),
            });
          }
          ...
          if (currentLanguage === 'dzo' && originalKeywords.length > 0) {
            ...
            if (isMountedRef.current && results?.length === originalKeywords.length) {
          ...
          const symbolsData: DisplayedSymbolData[] = originalKeywords.map((originalKw, idx) => ({
            id: `${symbolIdsPrefix}_${idx}_${originalKw.replace(/\s+/g, '_').toLowerCase()}`,
            keyword: originalKw,
          ...
          if (onCategoryNameChange) onCategoryNameChange(currentCategoryDisplayName);
      ...
      [ localCustomSymbols, apiStandardCategories, apiTimeContextSymbols, allDisplayCategories, isLoadingInitialData,
        currentLanguage, translateSymbolBatch, t, onCategoryNameChange]
    );

    useEffect(() => {
      if (!isLoadingInitialData) {
        loadAndTranslateSymbols(selectedCategoryKey);
      } else if (onCategoryNameChange && selectedCategoryKey === null && !loadingApiTimeContext && !loadingApiStandardCategories && !loadingLocalCustom) {
        onCategoryNameChange(t('home.contextualCategory'));
      }
    }, [isLoadingInitialData, selectedCategoryKey, loadAndTranslateSymbols, currentLanguage, onCategoryNameChange, t, loadingApiTimeContext, loadingApiStandardCategories, loadingLocalCustom]);
    ...
    const renderLeftItem = useCallback(({ item }: { item: DisplayedSymbolData }) => (
      ...
      {/* Dividing by 2 for margin around item, consistent with gridContentContainer padding */}
        <MemoizedSquareComponent
          ...
          size={itemWidth}
          onPress={(originalKeyword: any) => onSymbolPress(originalKeyword, item.imageUri)}
        />
      </View>
    ...
    const renderRightItem = useCallback(({ item: categoryInfo }: { item: CategoryInfo }) => {
      ...
      if (categoryKey === 'contextual') displayedName = t('home.contextualCategory');
      else if (categoryKey === 'custom') displayedName = t('home.customCategory');
      // Use a safe name for translation key, and provide a robust defaultValue
      else displayedName = t(`categories.${categoryInfo.name.toLowerCase()}`, { defaultValue: categoryInfo.name.charAt(0).toUpperCase() + categoryInfo.name.slice(1).toLowerCase() });
    ...
    const addSymbolToLocalCustomCategory = useCallback(async (keywordToAdd: string, imageUri?: string) => {
      if (!keywordToAdd.trim()) {
        Alert.alert(t('common.error'), t('home.errors.emptySymbolName')); return;
      }
      const keywordLower = keywordToAdd.toLowerCase();
      if (localCustomSymbols.some(cs => cs.name.toLowerCase() === keywordLower)) {
        Alert.alert(t('home.alreadyExistsAlertTitle'), t('home.customSymbolExists', { symbol: keywordToAdd })); return;
      }
      const newSymbol: CustomSymbolItem = {
        id: `custom-${Date.now()}-${Math.random().toString(36).substring(7)}`,
        name: keywordToAdd.trim(), imageUri,
      };
      if (isMountedRef.current) {
          ...
          <View style={styles.leftSide}>
            {showGridLoading ? (
              ...
              <FlatList
                ...
                ListEmptyComponent={<View style={styles.emptyContainer}><Text style={[styles.emptyListText, { color: theme.textSecondary }]}>{t('home.noSymbols')}</Text></View>}
                ...
                getItemLayout={(data, index) => ({ length: itemContainerHeight, offset: itemContainerHeight * Math.floor(index / numGridColumns), index })}
          ...
          <View style={styles.rightSide}>
            {showCategoryListLoading ? (
              ...
              <FlatList
                ...
                initialNumToRender={allDisplayCategories.length > 25 ? 25 : allDisplayCategories.length} 
              />
            )}
          </View>
        </View>

...
import {
    ...
    Easing, Platform, Text, Alert, ActivityIndicator // Added ActivityIndicator
} from 'react-native';
...
const HomeScreen = () => {
    ...
    // --- NEW: Loading state for LLM/API call ---
    ...
    // --- Animation & Timer Logic, TTS Init (Keep as before) ---
    ...
    // --- MODIFIED handleSpeakPress with API call ---
    const handleSpeakPress = useCallback(async () => {
        showAndResetTimer();
        if (!isTtsInitialized) { Alert.alert("TTS Error", "TTS not ready."); return; }
        if (isGeneratingPhrase || isTtsSpeaking) { if(isTtsSpeaking) Tts.stop(); return; }
        if (selectedItems.length === 0) return;

        const keywords = selectedItems.map(item => item.keyword);
        let sentenceToSpeak = keywords.join(' '); // Default/Fallback

        setIsGeneratingPhrase(true); // Start loading indicator

        try {
            console.log("Sending keywords to API:", keywords);
            const response = await axios.post(
                PHRASE_GENERATION_API_URL,
                { words: keywords }, // Payload matches the API spec
            ...
            if (response.data && typeof response.data.phrase === 'string') {
                ...
                // Clean up potential extra tokens like <|im_end|>
                generatedPhrase = generatedPhrase.replace(/<\|im_end\|>$/, '').trim();
                if (generatedPhrase) {
                     sentenceToSpeak = generatedPhrase; // Use API phrase
            ...
            // Keep sentenceToSpeak as the joined keywords (fallback)
        } finally {
            setIsGeneratingPhrase(false); // Stop loading indicator
        }

        // Proceed to speak (either generated phrase or fallback)
        if (sentenceToSpeak) { // Ensure there's something to speak
            console.log(`HomeScreen: Speaking - "${sentenceToSpeak}"`);
            try {
                // Apply TTS settings just before speaking
                ...
                setIsTtsSpeaking(false); // Ensure state is correct on error
    ...
    }, [selectedItems, isTtsSpeaking, ttsSettings, isTtsInitialized, showAndResetTimer, isGeneratingPhrase]); // Added isGeneratingPhrase
    ...
    const handleSearchSymbolSelect = useCallback((symbol: SearchSymbolInfo) => { setSelectedItems(prev => [...prev, { keyword: symbol.keyword, id: `${Date.now()}-${symbol.keyword}-${prev.length}` }]); const currentCategory = symbolGridRef.current?.selectedCategoryName; if (currentCategory && currentCategory !== 'Contextual' && currentCategory !== 'Custom') { symbolGridRef.current?.addKeywordToCategory(currentCategory, symbol.keyword); } else { console.log(`Cannot add to category: ${currentCategory}`); } showAndResetTimer(); }, [showAndResetTimer]);
        ...
        <SafeAreaView style={styles.safeArea} edges={['top', 'left', 'right']}>
            <TouchableWithoutFeedback onPress={showAndResetTimer} accessible={false}>
                 <View style={styles.container}>
                    ...
                    <IconInputComponent
                        ...
                        isSpeakDisabled={selectedItems.length === 0 || isTtsSpeaking || isGeneratingPhrase || !isTtsInitialized} // Disable during generation
                    ...
                    >
                        {/* Show loading indicator in input bar while generating */}
            ...
            <Animated.View style={[styles.bottomBarContainer, { transform: [{ translateY: bottomBarPosition }] }]} pointerEvents={isBarVisible ? 'auto' : 'none'}>
                <BottomBar
                    ...
                    // Pass gridLayout from context if needed by BottomBar itself

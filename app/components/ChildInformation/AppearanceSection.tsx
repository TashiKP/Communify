...
const AppearanceSection: React.FC<AppearanceSectionProps> = ({childData}) => {
  ...
  ): {display: string; percentage: number | undefined} => {
    if (value === undefined) {
  ...
  const renderAppearanceSetting = (
    ...
    <View style={styles.appearanceSettingItem}>
      <FontAwesomeIcon
        ...
        size={fonts.body}
        color={theme.primary}
        style={styles.appearanceSettingIcon}
      />
    ...
    <LinearGradient
      ...
      <View style={styles.appearanceSettingsContainer}>
        {renderAppearanceSetting(
          ...
          formatValue(
            childData?.appearanceSettings?.brightness
              ? childData.appearanceSettings.brightness / 100
              : undefined,
          ).display,
          faSun,
          formatValue(
        ...
        {renderAppearanceSetting(
          ...
          childData?.appearanceSettings?.font_size
            ? capitalize(childData.appearanceSettings.font_size)
            : 'N/A',
          faFont,
        )}
        {renderAppearanceSetting(
          t('childInformation.gridLayout'),
          childData?.appearanceSettings?.symbol_grid_layout
            ? capitalize(childData.appearanceSettings.symbol_grid_layout)
            : 'N/A',
          faTh,
        )}
        {renderAppearanceSetting(

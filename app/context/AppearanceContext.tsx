...
export const AppearanceProvider: FC<{children: ReactNode}> = ({children}) => {
  ...
  useEffect(() => {
    ...
    const loadSettings = async () => {
      try {
        const storedJson = await AsyncStorage.getItem(
        ...
        if (storedJson) {
          ...
          try {
            const parsedFullSettings: Partial<FullDisplaySettingsData> =
              JSON.parse(storedJson);

            const validTextSizes: TextSizeType[] = ['small', 'medium', 'large'];
            const validContrastModes: ContrastModeType[] = [
              'default',
              'high-contrast-light',
              'high-contrast-dark',
            ];
            ...
            if (typeof parsedFullSettings.darkModeEnabled === 'boolean') {
              loadedAppearanceSettings.darkModeEnabled =
                parsedFullSettings.darkModeEnabled;
            }
            if (
              parsedFullSettings.contrastMode &&
              validContrastModes.includes(parsedFullSettings.contrastMode)
            ) {
              loadedAppearanceSettings.contrastMode =
                parsedFullSettings.contrastMode;
            }
            if (
              parsedFullSettings.textSize &&
              validTextSizes.includes(parsedFullSettings.textSize)
            ) {
              loadedAppearanceSettings.textSize = parsedFullSettings.textSize;
            }
            if (
              typeof parsedFullSettings.brightness === 'number' &&
              parsedFullSettings.brightness >= 0 &&
              parsedFullSettings.brightness <= 100
            ) {
              loadedAppearanceSettings.brightness =
                parsedFullSettings.brightness;
            }
            // console.log('AppearanceContext: Loaded settings from storage:', loadedAppearanceSettings);
          } catch (parseError) {
            console.error(
              'AppearanceContext: Failed to parse stored display settings. Using defaults.',
              parseError,
            );
        ...
        console.error(
          'AppearanceContext: Failed to read display settings. Using defaults.',
          e,
        );
        if (isMountedRef.current) setSettings(defaultAppearanceSettings);
      } finally {
  ...
  const updateAppearanceSetting = useCallback(
    async <K extends keyof AppearanceSettings>(
      key: K,
      value: AppearanceSettings[K],
    ) => {
      ...
      try {
        ...
        let existingFullSettings: FullDisplaySettingsData = {
          ...defaultFullDisplaySettingsData,
        };
        if (storedJson) {
          try {
            existingFullSettings = {
              ...defaultFullDisplaySettingsData,
              ...JSON.parse(storedJson),
            };
          } catch (parseError) {
            console.error(
              'AppearanceContext: Corrupted existing settings before saving. Starting from defaults for save.',
              parseError,
            );
          }
        }
        ...
        await AsyncStorage.setItem(
          DISPLAY_SETTINGS_STORAGE_KEY,
          JSON.stringify(fullSettingsToSave),
        );
        // console.log('AppearanceContext: Settings saved to storage for key:', key);
      } catch (e) {
        console.error(
          'AppearanceContext: Failed to save setting to storage for key:',
          key,
          e,
        );
      }
    ...
    [settings],
  ); // Depends on current settings to merge correctly

  const theme = useMemo(
    () => getTheme(settings.darkModeEnabled, settings.contrastMode),
    [settings.darkModeEnabled, settings.contrastMode],
  );
  const fonts = useMemo(
    () => getFontSizes(settings.textSize),
    [settings.textSize],
  );

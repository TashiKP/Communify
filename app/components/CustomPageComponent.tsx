...
const calculateItemWidth = (
  ...
  const availableWidthForGrid =
    screenWidth - gridPadding * 2 - sectionGridPadding * 2;
  return Math.max(
    80,
    Math.floor((availableWidthForGrid - itemMargin * 2 * numCols) / numCols),
  );
};
...
const CustomPageComponent: React.FC<CustomPageComponentProps> = ({
  onBackPress,
  onSymbolsUpdate,
}) => {
  ...
  const [isDeletingSymbolId, setIsDeletingSymbolId] = useState<string | null>(
    null,
  );
  ...
  const numInternalColumns = useMemo(
    () => getNumColumns(gridLayout),
    [gridLayout],
  );
  const itemWidth = useMemo(
    () => calculateItemWidth(gridLayout, numInternalColumns),
    [gridLayout, numInternalColumns],
  );
      ...
      } catch (e) 
      {
        ...
        if (isMounted) {
          ...
          requestAnimationFrame(() => {
            initialLoadComplete.symbols = true;
          });
        }
      }
    };
    ...
    const loadCategories = async () => {
      ...
      try {
        ...
        if (isMounted) {
          ...
          const validCategories = Array.isArray(loadedCategories)
            ? loadedCategories
            : [];
  ...
  useEffect(() => {
    ...
    const save = async () => {
      try {
        ...
        Alert.alert(
          t('common.error'),
          t('customPage.errors.saveCategoriesFail'),
        );
      }
    };
    const timerId = setTimeout(save, 500);
  ...
  const sectionedListData = useMemo(() => {
    ...
    if (
      (uncategorizedSymbols && uncategorizedSymbols.length > 0) ||
      categories.length === 0
    ) {
      sections.push({
        id: null,
        name: uncategorizedLabel,
        symbols: uncategorizedSymbols || [],
      });
    }
    ...
    sortedCategories.forEach(category => {
      sections.push({
        id: category.id,
        name: category.name,
        symbols: groupedSymbols.get(category.id) || [],
      });
    });
  ...
  const handleAddNewCategoryInModal = useCallback(
    (name: string): CategoryItem | null => {
      ...
      Alert.alert(
        t('customPage.categoryAddedTitle'),
        t('customPage.categoryAddedMessage', {name: trimmedName}),
      );
      return newCategory;
    },
    [categories, t],
  ...
  const handleSaveSymbolFromModal = useCallback(
    (data: SymbolModalData, originalSymbolId?: string) => {
      if (modalMode === 'edit' && originalSymbolId) {
        setSymbols(prev =>
          prev.map(s => (s.id === originalSymbolId ? {...s, ...data} : s)),
        );
      } else {
    ...
    Alert.alert(
      ...
      [
        {text: t('common.cancel'), style: 'cancel'},
        {
          ...
          style: 'destructive',
          onPress: () => confirmDeleteSymbol(symbol.id),
        },
      ],
    );
  };
  ...
  const confirmDeleteSymbol = (symbolId: string) => {
    ...
    setTimeout(() => {
      // Simulate deletion delay / allow UI update
      if (isMountedRef.current) {
  ...
  const renderSectionItem = ({item: section}: {item: GridSection}) => {
    return (
      <CategorySection
        ...
        onEditSymbol={(symbol: SymbolItem | undefined) =>
          openSymbolModal('edit', symbol)
        }
  ...
  const styles = useMemo(
    () =>
      StyleSheet.create({
        ...
        pageContent: {
          // Was 'container' in original styles, renamed to avoid conflict
          flex: 1,
        ...
        <View style={styles.pageContent}>
          {isLoadingPage ? (
            ...
            <FlatList
              ...
              extraData={`${gridLayout}-${categories.length}-${
                symbols.length
              }-${JSON.stringify(expandedCategories)}-${
                i18n.language
              }-${itemWidth}-${isDeletingSymbolId}`}
            />
          )}
        </View>

...
interface ScreenTimeSectionProps {
  ...
  onSettingChange: <K extends keyof ParentalSettingsData>(
    key: K,
  ...
  ) => Promise<void>; // New prop for sending email
...
const ScreenTimeSection: React.FC<ScreenTimeSectionProps> = ({
  ...
  const switchStyles = useMemo(
    () => ({
      trackColor: {
        false: theme.disabled || '#767577',
        true: theme.secondary || '#81c784',
      },
      thumbColor:
        Platform.OS === 'android' ? theme.primary || '#007aff' : undefined,
      ios_backgroundColor: theme.disabled || '#767577',
    }),
    [theme],
  );

  const [reportedDailyLimit, setReportedDailyLimit] = useState<string | null>(
    null,
  );
  const [isDowntimeActiveCurrently, setIsDowntimeActiveCurrently] =
    useState(false);

  const handleGeneratedReport = useCallback(
    (event: ReportEvent) => {
      // 1. Default UI Alert & Console Log
      console.log(
        `[ParentalControlReport] ${event.type}: ${event.message}`,
        event.details || '',
      );
      Alert.alert(
        t(`parentalControls.reports.titles.${event.type}`, {
          defaultValue: 'Parental Control Update',
        }),
        event.message,
      );
      ...
      if (
        ...
        const emailSubject = t(
          `parentalControls.reports.emailSubjects.${event.type}`,
          {
            defaultValue: t(`parentalControls.reports.titles.${event.type}`, {
              defaultValue: `Update: ${event.type}`,
            }),
          },
        );
        ...
        if (event.details && Object.keys(event.details).length > 0) {
          emailBody += `${t(
            'parentalControls.reports.emailDetailsSectionTitle',
        ...
        sendEmailReport(
          ...
          .then(() => {
            console.log(
              `Email report for ${event.type} successfully queued for sending to:`,
              settings.notifyEmails.join(', '),
            );
          })
          .catch(error => {
            console.error(
              `Failed to queue email report for ${event.type}:`,
              error,
            );
            Alert.alert(
              t('parentalControls.reports.emailFailedTitle', {
                defaultValue: 'Email Report Failed',
              }),
              t('parentalControls.reports.emailFailedMessage', {
                defaultValue: `Could not send email notification: ${
        ...
        sendEmailReport &&
        (!settings.notifyEmails || settings.notifyEmails.length === 0)
      ) {
        console.log(
          `Email reporting is set up, but no notification email addresses are configured for event type: ${event.type}.`,
        );
      }
    ...
    [t, sendEmailReport, settings, onReport, i18n.language],
  ); // settings includes notifyEmails
    ...
    if (!isNaN(limitNum) && limitNum > 0) {
      if (limitStr !== reportedDailyLimit) {
        handleGeneratedReport({
          ...
          message: t('parentalControls.reports.dailyLimitSet', {
            hours: limitStr,
          }),
  ...
  useEffect(() => {
    const checkDowntime = () => {
      ...
      if (
        ...
        currentApiDay &&
        settings.downtimeDays.includes(currentApiDay)
      ) {
        ...
        if (!isNaN(downtimeStartMinutes) && !isNaN(downtimeEndMinutes)) {
          if (downtimeStartMinutes <= downtimeEndMinutes) {
            // Same day
            if (
              currentTimeMinutes >= downtimeStartMinutes &&
              currentTimeMinutes < downtimeEndMinutes
            ) {
      ...
      if (isDeviceCurrentlyInDowntime !== isDowntimeActiveCurrently) {
        ...
        const eventType = isDeviceCurrentlyInDowntime
          ? 'downtime_active'
          : 'downtime_inactive';
        const message = isDeviceCurrentlyInDowntime
          ? t('parentalControls.reports.downtimeActive', {
              until: settings.downtimeEnd,
            })
          : t('parentalControls.reports.downtimeInactive');
        const details = isDeviceCurrentlyInDowntime
          ? {
              activeUntil: settings.downtimeEnd,
              scheduledStart: settings.downtimeStart,
            }
          : {
              lastScheduledStart: settings.downtimeStart,
              lastScheduledEnd: settings.downtimeEnd,
            };
  ...
  const getTranslatedDay = (dayKey: DayOfWeek): string => {
    return t(`parentalControls.screenTime.days.${dayKey.toLowerCase()}`, {
      defaultValue: dayKey,
    });
  };
    ...
    <View style={[styles.defaultSectionCard, sectionStyle]}>
      ...
      <View style={[styles.defaultCardHeader, headerStyle]}>
        ...
        <Text style={[styles.defaultSectionTitle, titleStyle]}>
          {t('parentalControls.screenTime.sectionTitle')}
        </Text>
      </View>
      ...
      <View style={styles.settingRow}>
        ...
        <View style={styles.timeInputContainer}>
          <TextInput
            ...
            onChangeText={text =>
              onSettingChange('dailyLimitHours', text.replace(/[^0-9.]/g, ''))
            }
            ...
            accessibilityLabel={t(
              'parentalControls.screenTime.dailyLimitAccessibilityLabel',
            )}
          />
          <Text
            style={[
      ...
      <View style={styles.settingRow}>
        ...
        <Switch
          ...
          {...switchStyles}
          accessibilityLabel={t(
            'parentalControls.screenTime.downtimeToggleAccessibilityLabel',
          )}
      ...
      {settings.downtimeEnabled && (
        <View style={styles.downtimeDetails}>
          <Text
            style={[styles.fieldLabel, {color: theme.textSecondary || '#555'}]}>
            ...
            {daysOfWeek.map(day => {
              ...
              return (
                <TouchableOpacity
                  ...
                  style={[
                    styles.dayButton,
                    isDaySelected && styles.dayButtonSelected,
                  ]}
                  onPress={() => onDayToggle(day)}
                  activeOpacity={0.7}
                  accessibilityLabel={t(
                    'parentalControls.screenTime.dayToggleAccessibilityLabel',
                    {day: translatedDay},
                  )}
                  ...
                  <Text
                    style={[
                      ...
                      isDaySelected && styles.dayButtonTextSelected,
                    ]}>
          ...
          <View style={styles.timeSelectionRow}>
            <TouchableOpacity
              ...
              activeOpacity={0.7}
              accessibilityLabel={t(
                'parentalControls.screenTime.downtimeStartAccessibilityLabel',
                {time: settings.downtimeStart},
              )}
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.7}
              accessibilityLabel={t(
                'parentalControls.screenTime.downtimeEndAccessibilityLabel',
                {time: settings.downtimeEnd},
              )}
              accessibilityRole="button">
              <Text
                style={[

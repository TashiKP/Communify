...
import {
    View, Text, TouchableOpacity, StyleSheet, Modal, ScrollView, Image, TextInput,
    ActivityIndicator, Keyboard, TouchableWithoutFeedback, Platform, Alert
} from 'react-native';
import { FontAwesomeIcon } from '@fortawesome/react-native-fontawesome';
import {
    faTimes, faImage, faCheck, faExclamationTriangle, faFolderPlus, faTrash
...
const createStyles = (theme: ThemeColors, fonts: FontSizes, language: string) => {
    ...
    return StyleSheet.create({
        ...
        pickerItem: { // For Picker.Item specifically
            ...bodyStyles,
            fontWeight: '500',
            color: theme.text,
        },
...
const SymbolModal: React.FC<SymbolModalProps> = ({
    ...
    const [modalSelectedCategoryId, setModalSelectedCategoryId] = useState<string | null | undefined>(null); // Default to null (Uncategorized)
    ...
    const pickImage = () => {
        ...
        launchImageLibrary(options, (response: ImagePickerResponse) => {
            ...
            if (response.errorCode) {
                ...
                Alert.alert(t('common.error'), response.errorMessage || t('customPage.errors.imageSelectFail'));
                return;
            }
    ...
    const handleInternalAddNewCategory = () => {
        ...
        if (!trimmedName) {
            Alert.alert(t('customPage.errors.invalidCategoryNameTitle'), t('customPage.errors.invalidCategoryNameMessage'));
            return;
        }
    ...
    const handleInternalSaveSymbol = () => {
        ...
        if (!name) {
            Alert.alert(t('customPage.errors.validationErrorTitle'), t('customPage.errors.symbolNameRequired'));
            return;
        }
    ...
    const currentPickerValue = modalSelectedCategoryId === undefined ? null : modalSelectedCategoryId;
                        ...
                        <View style={styles.modalContent}>
                            <View style={styles.modalHeader}>
                                <Text style={styles.modalTitleText}>
                                    {mode === 'add' ? t('customPage.modal.addTitle') : t('customPage.modal.editTitle')}
                                </Text>
                                <TouchableOpacity onPress={onClose} style={styles.modalCloseButton} activeOpacity={0.6} accessibilityLabel={t('customPage.modal.closeAccessibilityLabel')} accessibilityRole="button">
                            ...
                            <ScrollView contentContainerStyle={styles.modalBody} keyboardShouldPersistTaps="handled">
                                ...
                                <TouchableOpacity style={styles.modalImagePicker} onPress={pickImage} activeOpacity={0.6} accessibilityLabel={t('customPage.modal.chooseImageButton')} accessibilityRole="button">
                                    {modalImageUri ? (
                                        <Image source={{ uri: modalImageUri }} style={styles.modalPickedImage} accessibilityLabel="" />
                                    ) : (
                                ...
                                {showAddCategoryInput && (
                                    <View style={styles.addCategoryInputContainer}>
                                        ...
                                        />
                                        <TouchableOpacity
                                            style={[styles.addCategoryConfirmButton, !newCategoryName.trim() && styles.modalButtonDisabled, { backgroundColor: newCategoryName.trim() ? theme.primary : theme.disabled }]}
                                            onPress={handleInternalAddNewCategory}
                                            disabled={!newCategoryName.trim()}
                                            activeOpacity={0.6}
                                            accessibilityLabel={t('customPage.modal.confirmAddCategory')}
                                            accessibilityRole="button"
                                        >
                                ...
                                <TouchableOpacity
                                    ...
                                    activeOpacity={0.6}
                                    accessibilityLabel={mode === 'add' ? t('customPage.modal.addSymbolButton') : t('customPage.modal.saveChangesButton')}
                                    accessibilityRole="button"
                                >
                                    {isSavingModal ? (
                                        <ActivityIndicator size="small" color={theme.white} style={styles.buttonIcon} />
                                    ) : (
                                        <FontAwesomeIcon icon={faCheck} size={fonts.body * 1.2} color={theme.white} style={styles.buttonIcon} />
                                    )}
                                    <Text style={styles.modalButtonText}>
                                        {mode === 'add' ? t('customPage.modal.addSymbolButton') : t('customPage.modal.saveChangesButton')}
                                    </Text>
                                </TouchableOpacity>

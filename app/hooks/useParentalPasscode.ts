...
export const useParentalPasscode = ({
  ...
  const checkPasscodeStatus = useCallback(async () => {
    ...
    try {
      ...
      if (isMountedRef.current) {
        ...
        console.log(
          'useParentalPasscode: Passcode status checked. Exists:',
          backendStatus.exists,
        );
      }
    } catch (error) {
      console.error(
        'useParentalPasscode: Failed to check passcode status:',
        error,
      );
      if (isMountedRef.current) {
        ...
        Alert.alert(
          t('common.error'),
          t('parentalControls.errors.passcodeStatusFail'),
        );
      }
    } finally {
      if (isMountedRef.current) {
  ...
  const handleSetOrUpdatePasscode = useCallback(async () => {
    ...
    try {
      if (passcodeExists) {
        const keychainVerified = await KeychainService.verifyPasscode(
          currentPasscode,
        );
        if (!keychainVerified) {
          setPasscodeError(
            t('parentalControls.passcode.errorIncorrectCurrent'),
          );
      ...
      if (!apiResponse.success) {
        setPasscodeError(
          apiResponse.message ||
            t('parentalControls.passcode.errorApiSaveFailed'),
        );
        setIsSettingPasscode(false);
        return;
      }
      ...
      setPasscodeError(
        apiError.message || t('parentalControls.passcode.errorUnexpected'),
      );
    } finally {
      if (isMountedRef.current) {
        setIsSettingPasscode(false);
    ...
    currentPasscode,
    newPasscode,
    confirmPasscode, // State values, will cause regen, fine.
    t,
    togglePasscodeSetup,
    onPasscodeStateChange,
    updateRequirePasscodeSetting, // Callbacks/stable values.
  ]);
  ...
  const handleRemovePasscodeClick = useCallback(async () => {
    ...
    if (!currentPasscode) {
      setPasscodeError(
        t('parentalControls.passcode.errorEnterCurrentToRemove'),
      );
      return;
    }
        ...
        {text: t('common.cancel'), style: 'cancel'},
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              const apiResponse =
                await apiService.removeParentalPasscodeOnBackend(
                  currentPasscode,
                );
              if (!apiResponse.success) {
                setPasscodeError(
                  apiResponse.message ||
                    t('parentalControls.passcode.errorApiRemoveFailed'),
                );
                setIsSettingPasscode(false);
                return;
              }
              ...
              setTimeout(() => {
                if (isMountedRef.current) togglePasscodeSetup();
              }, 1500);
            } catch (error) {
              const apiError = handleApiError(error);
              setPasscodeError(
                apiError.message ||
                  t('parentalControls.passcode.errorUnexpected'),
              );
            } finally {
    ...
    togglePasscodeSetup,
    onPasscodeStateChange,
    updateRequirePasscodeSetting, // Callbacks/stable
  ]);
  ...
  return {
    ...
    showPasscodeSetup,
    currentPasscode,
    setCurrentPasscode, // Expose setters for the form
    newPasscode,
    setNewPasscode,
    confirmPasscode,
    setConfirmPasscode,
